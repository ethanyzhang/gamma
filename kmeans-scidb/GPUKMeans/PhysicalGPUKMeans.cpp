...
/*
 * @file PhysicalGPUKMeans.cpp
 * @author Yiqun Zhang <zhangyiqun9164@gmail.com>
 *
 * @brief Physical part of the GPUKMeans SciDB operator.
 */
...
namespace scidb {
class PhysicalGPUKMeans : public PhysicalOperator {
public:
  ofstream log;
  ...
  void initOperator(shared_ptr<Array> inputArray, shared_ptr<Query> query) {
    ...
    stringstream ss;
    ss << getenv("HOME") << "/GPUkmeans-instance-" << query->getInstanceID() << ".log";
  ...
  void iterativeKmeans(shared_ptr<Array> inputArray, shared_ptr<Query> query) {
    ...
    for (int32_t i=0; i<imax; i++) {
      ...
      while ( ! inputArrayIter->end()) {
        ...
        sumDistance += assignCluster(memChunk, centroids, gamma, k, d, nChunkSize);
        ++(*inputArrayIter);
      }
  ...
  shared_ptr<Array> writeResults(shared_ptr<Query> query) {
    ...
    sort(weights, weights+k, &PhysicalGPUKMeans::sortWeightById);
  ...
  PhysicalGPUKMeans(string const& logicalName,
               string const& physicalName,
               Parameters const& parameters,
               ArrayDesc const& schema):
    PhysicalOperator(logicalName, physicalName, parameters, schema) {
    n = d = dStart = centroidsLen = gammaLen = nChunkSize = 0;
    k = imax = 0;
    omega = sumDistance = 0;
    coordinatorInstanceId = 0;
    lastAverageDistance = numeric_limits<double>::max();
    centroidsStorage = gammaStorage = variancesStorage = memChunkStorage = NULL;
    weights = NULL;
    centroids = gamma = variances = memChunk = NULL;
...
REGISTER_PHYSICAL_OPERATOR_FACTORY(PhysicalGPUKMeans, "GPUKMeans", "PhysicalGPUKMeans");
