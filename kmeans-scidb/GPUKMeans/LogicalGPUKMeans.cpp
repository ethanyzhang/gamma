...
/*
 * @file LogicalGPUKMeans.cpp
 * @author Yiqun Zhang <zhangyiqun9164@gmail.com>
 *
 * @brief Logical part of the GPUKMeans SciDB operator.
 */
...
class LogicalGPUKMeans : public LogicalOperator {
public:

  LogicalGPUKMeans(const string& logicalName, const string& alias):
  ...
  ArrayDesc inferSchema(vector<ArrayDesc> schemas, shared_ptr<Query> query) {
    ...
    if (inputDims.size() != 2) {
      throw SYSTEM_EXCEPTION(SCIDB_SE_OPERATOR, SCIDB_LE_INVALID_FUNCTION_ARGUMENT)
          << "Operator GPUKMeans accepts an array with exactly 2 dimensions.";
    }
    ...
    if (inputSchema.getAttributes(true).size() != 1 ||
      ...
      throw SYSTEM_EXCEPTION(SCIDB_SE_OPERATOR, SCIDB_LE_INVALID_FUNCTION_ARGUMENT)
          << "Operator GPUKMeans accepts an array with one attribute of type double";
    }
    ...
    outputDims.push_back( DimensionDesc("j", 1, 2*d+1, 2*d+1, 0) ); // W: 1, C:d, R:d
    return ArrayDesc("GPUKMeans_" + inputSchema.getName(), outputAttributes, outputDims, defaultPartitioning());
  }
};

REGISTER_LOGICAL_OPERATOR_FACTORY(LogicalGPUKMeans, "GPUKMeans");
